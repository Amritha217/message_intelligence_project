...
def save_data_cache(path="output/assistant_data_cache.json"):
    cache = build_data_cache()
    with open(path, "w") as f:
        json.dump(cache, f, indent=2)
    print(f"Saved assistant data cache -> {path}")


def load_data_cache(path="output/assistant_data_cache.json"):
    with open(path) as f:
        cache = json.load(f)
    cache["common_words"] = set(cache["common_words"])
    cache["messages"] = {}  
    return cache
